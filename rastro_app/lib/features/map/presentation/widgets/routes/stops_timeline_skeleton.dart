...
class StopsTimelineSkeleton extends StatefulWidget {
  final int itemCount;

  const StopsTimelineSkeleton({this.itemCount = 4, super.key});

  @override
  State<StopsTimelineSkeleton> createState() => _StopsTimelineSkeletonState();
}

class _StopsTimelineSkeletonState extends State<StopsTimelineSkeleton>
  ...
  Widget build(BuildContext context) {
    ...
    // Vary label widths so it looks natural
    ...
    return AnimatedBuilder(
      ...
      builder: (_, __) {
        ...
        // Build all rows (1 fit-route + itemCount stops)
        final total = 1 + widget.itemCount;

        final rows = List.generate(total, (i) {
          final isFirst = i == 0;
          final isLast = i == total - 1;
          final isRouteItem = i == 0;

          final dot = isRouteItem
              ? bone(26, 26, radius: 13) // circle for fit-route
              : bone(
                  i == 1 || isLast ? 12 : 8,
                  i == 1 || isLast ? 12 : 8,
                  radius: 6,
